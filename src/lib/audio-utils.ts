 ...
 */
export function generateAudioFilename(podcastId: string, sectionId: string): string {
 ...
 */
export async function getAudioUrl(podcastId: string, sectionId: string): Promise<string> {
  const newFilename = generateAudioFilename(podcastId, sectionId);
  const oldFilename = `${podcastId}-${sectionId}.wav`;
  
  // Try the new format first
  try {
    const response = await fetch(`/api/audio/${newFilename}`, { method: 'HEAD' });
    if (response.ok) {
 ...
 */
export function extractIdsFromFilename(filename: string): { podcastId: string; sectionId: string } | null {
  // Remove extension
