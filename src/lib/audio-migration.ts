 ...
 */
export async function migrateAudioFiles(): Promise<{ success: number; failed: number }> {
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (file.length === 12 && file.endsWith('.wav')) { // 8 chars + .wav = 12
...
export async function checkMigrationNeeded(): Promise<boolean> {
  ...
  try {
    ...
    const oldFormatFiles = files.filter(file => 
      file.endsWith('.wav') && file.length > 12
    );
